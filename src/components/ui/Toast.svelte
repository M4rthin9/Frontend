...
<div class="pointer-events-none fixed right-4 top-4 z-[60] flex w-full max-w-sm flex-col gap-2">
  {#each ui.toasts as toast (toast.id)}
    <div transition:fly={{ x: 24, duration: 200 }} class={toastClasses(toast.type)}>
      ...
      <p class="flex-1 text-sm font-medium text-text-primary">{toast.message}</p>
      <button
        type="button"
        class="shrink-0 rounded-lg p-1 text-text-tertiary transition-all duration-200 hover:text-text-primary hover:bg-background-subtle"
